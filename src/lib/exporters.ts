...
export const exportAllModulesToXlsx = async (startDate?: string, endDate?: string) => {
  ...
  sheetExtintores.columns = [
    ...
    { header: 'SONDA', key: 'sonda', width: 18 },
    { header: 'MANÓMETRO', key: 'manometro', width: 18 },
  ...
  flota.forEach((bus) => {
    ...
    const row = sheetExtintores.addRow({
      ...
      vencimiento: ext?.vencimiento_mes && ext?.vencimiento_anio ? `${ext.vencimiento_mes}/${ext.vencimiento_anio}` : '-',
      certificacion: ext?.certificacion || '-',
      sonda: ext?.sonda || '-',
